...
class UserInfoUpdate extends StatelessWidget {
  ...
  String phone = '010-1234-5678';
    ...
    // 초기 더미 데이터 설정
    nicknameController.text = vmhandler.curUser.nickName;
    emailController.text = vmhandler.curUser.eMail;
    phoneController.text = vmhandler.curUser.phone;

    return GetBuilder<Vmhandler>(
      builder: (controller) {
        return Scaffold(
              body: Stack(
                children: [
                                        ...
                                        Column(
                                          children: [
                                            ...
                                            ?CircleAvatar(
                                              radius: 50,
                                              backgroundImage:
                                              vmhandler.curUser.profileImage == null?
                                              AssetImage('images/co2.png')
                                              :NetworkImage("http://127.0.0.1:8000/user/view/${vmhandler.curUser.profileImage!}"),
                                            ) 
                                            :CircleAvatar(
                                              radius: 50,
                                              backgroundImage: FileImage(File(vmhandler.imageFile!.path)),
                                            ),
                                            TextButton(
                                              onPressed: () {
                                                vmhandler.userImagePicker(ImageSource.gallery);
                                              },
                                              child: const Text(
                                                '사진 수정',
                                                style: TextStyle(
